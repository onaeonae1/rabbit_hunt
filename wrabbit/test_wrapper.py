...
def redis_subscribe(original_tag: int, ch: Channel, callback: Callable):
...
def partial_ack(ch: Channel, delivery_tag: int, __future):
    print("SDFDFDF")
...
class RabbitConfig:
    ...
    def receiver_callback(
        ...
        if res is not None:
            ...
            ch.connection.add_callback_threadsafe(ack_message_callback)

            # Executor 에 SUBMIT -> REDIS WATCH 를 하는 친구여야 한다
            future = execution_pool.submit(
                redis_subscribe, ch, method.delivery_tag, ack_message_callback
            )
            future.done
    ...
    def ack_callback(self, ch: Channel, delivery_tag: str, origin: str = "DEFAULT"):
        delivery_tag = int(delivery_tag)
        if self.verbose:
